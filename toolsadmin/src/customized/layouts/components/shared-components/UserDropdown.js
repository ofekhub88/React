...
const UserDropdown = props => {
  ...
  let RequestOptions = {...config.requestOptions}
      ...
      //+userData.username
  fetch(authConfig.photoEndpoint+"?username="+userData.username,RequestOptions).
    then(response => {
      if (response.ok) {
        return response.blob();
      } else {
        throw new Error('Unable to retrieve thumbnail photo');
      }
    })
    .then(blob => {
      // Convert the thumbnail photo data to a URL
      const url = URL.createObjectURL(blob);
      setThumbnailPhotoUrl(url);
    })
    .catch(error => {
      console.error(error);
    });
      ...
      >
        <Box sx={{ py: 1.75, px: 6 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            >
              <Avatar alt={userData.fullName} src={thumbnailPhotoUrl} sx={{ width: '2.5rem', height: '2.5rem' }} />
            </Badge>
